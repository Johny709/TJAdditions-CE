...
public class LinkedPosInfoProvider extends CapabilityInfoProvider<LinkPos> {
    ...
    protected void addProbeInfo(LinkPos capability, IProbeInfo probeInfo, TileEntity tileEntity, EnumFacing enumFacing) {
        ...
        for (int i = pageIndex; i < pageIndex + pageSize && i < size; i++) {
            ...
            final MetaTileEntity mte = entity instanceof MetaTileEntityHolder ? ((MetaTileEntityHolder) entity).getMetaTileEntity() : null;
            ...
            final ItemStack stack = mte != null ? mte.getStackForm() : new ItemStack(entity.getBlockType(), 1, entity.getBlockMetadata());
            nameInfo.item(stack);
            nameInfo.text(TextStyleClass.INFO + stack.getDisplayName());

            final IProbeInfo posInfo = probeInfo.vertical(probeInfo.defaultLayoutStyle().alignment(ElementAlignment.ALIGN_TOPLEFT));
            final int x = mte != null ? mte.getPos().getX() : entity.getPos().getX();
            final int y = mte != null ? mte.getPos().getY() : entity.getPos().getY();
            final int z = mte != null ? mte.getPos().getZ() : entity.getPos().getZ();
            posInfo.element(new ElementTJText(String.format("{*tj.machine.universal.linked.dimension[*%s;%s*]*}",
                    worldType.getName(), TJValues.thousandFormat.format(worldID))));
            posInfo.element(new ElementTJText(String.format("{*tj.machine.universal.linked.pos[*%s;%s;%s*]*}",
                    TJValues.thousandFormat.format(x), TJValues.thousandFormat.format(y), TJValues.thousandFormat.format(z))));
        }
    }
