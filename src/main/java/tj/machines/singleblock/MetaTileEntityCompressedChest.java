...
public class MetaTileEntityCompressedChest extends MetaTileEntity implements IFastRenderMetaTileEntity {
    ...
    public void addInformation(ItemStack stack, @Nullable World player, List<String> tooltip, boolean advanced) {
        ...
        tooltip.add(I18n.format("machine.universal.stack", TJValues.thousandFormat.format(this.isInfinite ? Integer.MAX_VALUE : 64)));
        tooltip.add(I18n.format("machine.universal.slots", TJValues.thousandFormat.format(ROW_SIZE * AMOUNT_OF_ROWS)));
        ...
        if (itemList.tagCount() > 0)
            tooltip.add(I18n.format("tj.machine.compressed_chest.slot_filled", TJValues.thousandFormat.format(itemList.tagCount()), TJValues.thousandFormat.format(ROW_SIZE * AMOUNT_OF_ROWS)));
        TooltipHelper.shiftText(tooltip, tip -> {
            TooltipHelper.pageText(tip, size, (tip1, tooltipHandler) -> {
                ...
                for (int i = start; i < Math.min(itemList.tagCount(), start + 10); i++) {
                    ...
                    tip1.add(I18n.format("tj.machine.compressed_chest.slot", TJValues.thousandFormat.format(itemCompound.getInteger("Slot")), itemStack.getDisplayName(), TJValues.thousandFormat.format(itemStack.getCount())));
                }
            });
        });
    }
