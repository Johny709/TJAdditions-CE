...
public class MetaTileEntityEnderBatteryTower extends ExtendableMultiblockController implements IEnderNotifiable<BasicEnergyHandler>, IProgressBar, IBatteryHandler {
    ...
    protected void addTabs(WidgetTabBuilder tabBuilder, EntityPlayer player) {
        ...
        String[] search = {"", "", ""};
        String[] playerName = {""};
        tabBuilder.addTab(this.getMetaFullName(), this.getStackForm(), tab -> this.createChannelTab(tab, player, searchResults, patternFlags, search))
        ...
        widget.accept(new ProgressWidget(this::getEnergyStored, 12, 40, 18, 18) {
            ...
            public void drawInForeground(int mouseX, int mouseY) {
                if(isMouseOverElement(mouseX, mouseY)) {
                    List<String> hoverList = Collections.singletonList(net.minecraft.client.resources.I18n.format("machine.universal.energy.stored", TJValues.thousandFormat.format(this.energyStored), TJValues.thousandFormat.format(this.energyCapacity)));
    ...
    private Consumer<GUIDisplayBuilder> addChannelDisplayText(int[] searchResults, int[][] patternFlags, String[] search) {
        return (builder) -> {
            int results = 0;
            builder.addTextComponent(new TextComponentString("§l" + TextUtils.translate("tj.multiblock.tab.channels") + "§r(§e" + searchResults[0] + "§r/§e" + this.getEnderProfile().getChannels().size() + "§r)"));
    ...
    private Consumer<GUIDisplayBuilder> addFrequencyDisplayText(int[] searchResults, int[][] patternFlags, String[] search) {
        return (builder) -> {
            int results = 0;
            builder.addTextComponent(new TextComponentString("§l" + TextUtils.translate("tj.multiblock.tab.frequencies") + "§r(§e" + searchResults[1] + "§r/§e" + this.getPlayerMap().size() + "§r)"));
    ...
    protected void addDisplayText(GUIDisplayBuilder builder) {
        ...
        builder.addEnergyStoredLine(this.inputEnergyContainer.getEnergyStored(), this.inputEnergyContainer.getEnergyCapacity())
                .customLine(text -> {
                    text.addTextComponent(new TextComponentTranslation("tj.multiblock.ender_battery_tower.energy_inserted", TJValues.thousandFormat.format(this.workableHandler.getEnergyInserted()))
                    ...
                    text.addTextComponent(new TextComponentTranslation("tj.multiblock.ender_battery_tower.energy_extracted",TJValues.thousandFormat.format (this.workableHandler.getEnergyExtracted()))
                    ...
                    text.addTextComponent(new TextComponentTranslation("tj.multiblock.ender_battery_tower.last_energy_inserted", TJValues.thousandFormat.format(this.workableHandler.getLastEnergyInserted()))
                    ...
                    text.addTextComponent(new TextComponentTranslation("tj.multiblock.ender_battery_tower.last_energy_extracted", TJValues.thousandFormat.format(this.workableHandler.getLastEnergyExtracted()))
