...
public class MetaTileEntityMegaAdvancedFusion extends TJRecipeMapMultiblockController implements IHeatInfo, IProgressBar, IFusionHandler {
    ...
    public static class MegaFusionRecipeLogic extends MegaRecipeLogic<IFusionHandler> {
        ...
        protected int checkFluidInputsAmount(int parallels, Recipe recipe) {
            ...
            for (int i = 0; i < size; i++) {
                final FluidStack fluidStack = ((IGTRecipe) recipe).getMergedFluidInputs().get(i).getFluidStack();
        ...
        protected void consumeFluidInputs(int parallels, Recipe recipe) {
            ...
            final int size = ((IGTRecipe) recipe).getMergedFluidInputs().size();
            for (int i = 0; i < size; i++) {
                FluidStack fluidStack = ((IGTRecipe) recipe).getMergedFluidInputs().get(i).getFluidStack();
