...
public class LargeWirelessEnergyWorkableHandler extends AbstractWorkableHandler<IMachineHandler> {
    ...
    public void initialize(int mode) {
        ...
        this.mode = (byte) mode;
        int linkAmount = this.handler.getTier() * 2;
        ...
        if (RFContainer != null) {
            if (this.mode == 0) {
                ...
                if (RFContainer.getEnergyStored() < 1 || energyRemainingToFill != 0) {
                    ...
                    this.handler.getInputEnergyContainer().removeEnergy(energyInserted / 4);
                }
            } else {
        ...
        if (EUContainer != null) {
            if (this.mode == 0) {
                ...
                if (EUContainer.getEnergyStored() < 1 || energyRemainingToFill != 0) {
                    ...
                    this.handler.getInputEnergyContainer().removeEnergy(energyInserted);
                }
            } else {
                long energyRemainingToFill = this.handler.getOutputEnergyContainer().getEnergyCapacity() - this.handler.getOutputEnergyContainer().getEnergyStored();
                if (this.handler.getOutputEnergyContainer().getEnergyStored() < 1 || energyRemainingToFill != 0) {
                    long energyExtracted = EUContainer.removeEnergy(energyToAdd);
                    this.handler.getOutputEnergyContainer().addEnergy(Math.abs(energyExtracted));
                }
            }
        }
    }
    ...
    public void updateTotalEnergyPerTick() {
        ...
        this.energyPerTick = (long) (Math.pow(4, this.handler.getTier()) * 8) * amps;
        this.metaTileEntity.markDirty();
    }
