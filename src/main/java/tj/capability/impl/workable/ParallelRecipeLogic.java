    ...
    protected boolean startRecipe(int i) {
        ...
        if (this.isDistinct) {
            ...
            if (recipe == null && (this.recipeRecheck[i] || this.checkRecipeInputsDirty(itemInputs, this.handler.getImportFluidTank()))) {
                ...
                } else recipe = this.handler.getRecipeMap().searchRecipe(this.handler.getMaxVoltage(), itemInputs, this.handler.getImportFluidTank(), this.getMinTankCapacity(this.handler.getExportFluidTank()), true);
                if (recipe != null) {
