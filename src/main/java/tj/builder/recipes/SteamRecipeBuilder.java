...
public class SteamRecipeBuilder extends RecipeBuilder<SteamRecipeBuilder> {
    ...
    protected EnumValidationResult validate() {
        if (EUt < 0) {
            TJ.logger.error("EU/t cannot be less than 0", new IllegalArgumentException());
            recipeStatus = EnumValidationResult.INVALID;
        }
        if (duration <= 0) {
            TJ.logger.error("Duration cannot be less or equal to 0", new IllegalArgumentException());
            recipeStatus = EnumValidationResult.INVALID;
        }
        if (recipeStatus == EnumValidationResult.INVALID) {
            TJ.logger.error("Invalid recipe, read the errors above: {}", this);
        }
        return recipeStatus;
    }
}
