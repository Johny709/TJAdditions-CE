...
public class GaugeDropperBehavior implements IItemBehaviour, ItemUIFactory, IItemCapabilityProvider {
    ...
    public ModularUI createUI(PlayerInventoryHolder holder, EntityPlayer player) {
        ...
        return ModularUI.defaultBuilder()
                .widget(new NewTextFieldWidget<>(27, 40, 119, 18, true, () -> String.valueOf(compound.getInteger("capacity")), (text, id) -> compound.setInteger("capacity", Math.min(1000, Integer.parseInt(text))))
                        .setValidator(str -> Pattern.compile("\\*?[0-9_]*\\*?").matcher(str).matches())
                        .setUpdateOnTyping(true)
                        .setMaxStringLength(5))
                .widget(new ToggleButtonWidget(7, 40, 18, 18, PLUS_BUTTON, () -> false, plus -> compound.setInteger("capacity", Math.min(1000, compound.getInteger("capacity") * 2))))
                .widget(new ToggleButtonWidget(151, 40, 18, 18, MINUS_BUTTON, () -> false, minus -> compound.setInteger("capacity", Math.max(1, compound.getInteger("capacity") / 2))))
                .widget(new ToggleButtonWidget(151, 58, 18, 18, RESET_BUTTON, () -> false, reset -> compound.setInteger("capacity", 1000)))
    ...
    public ICapabilityProvider createProvider(ItemStack itemStack) {
        return new FluidHandlerItemStack(itemStack, 1000) {
            ...
            public int fill(FluidStack resource, boolean doFill) {
                ...
                if (contained == null) {
                    final int fillAmount = Math.min(resource.amount, Math.min(this.capacity, capacity));
                    if (doFill) {
